...
// 实时心率传输服务
static NSString * const kRealTimeServiceUUID = @"180D";
// 实时心率传输特征值
static NSString *const KReadTimeCharacteristicUUID = @"2A37";
...
@property (nonatomic,strong)CBUUID *readTimeTeadUUID;
...
-(void)registerBlueToothManager
{
    ...
    self.readUUID= [CBUUID UUIDWithString:kReadCharacteristicUUID];
    self.writeUUID = [CBUUID UUIDWithString:kWriteCharacteristicUUID];
    self.readTimeServiceUUID = [CBUUID UUIDWithString:kRealTimeServiceUUID];
    self.readTimeTeadUUID = [CBUUID UUIDWithString:KReadTimeCharacteristicUUID];
}
...
+(instancetype)instance{
    ...
    lifeBitBools.readTimeServiceUUID = [CBUUID UUIDWithString:kRealTimeServiceUUID];
    lifeBitBools.readTimeTeadUUID = [CBUUID UUIDWithString:KReadTimeCharacteristicUUID];
...
-(void)cancelAllBluePeripheral{
    ...
    NSArray* arr = [self.manager retrieveConnectedPeripheralsWithServices:@[self.serviceUUID,self.readTimeServiceUUID]];
...
//    LBLog(@"%@ 初始连接成功",peripheral.name);
    for (LBWatchPerModel* watch in self.scanWatchArray) {
        if (watch.peripheral == peripheral) {
            ...
            peripheral.delegate = watch;
            [peripheral discoverServices:@[self.serviceUUID,self.readTimeServiceUUID]];
